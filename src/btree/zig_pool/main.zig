...
fn createTree(pool: *std.heap.MemoryPool(Node), allocator: std.mem.Allocator, depth: i32) !*Node {
    const node = try pool.create(allocator);
    if (depth > 0) {
        node.left = try createTree(pool, allocator, depth - 1);
        node.right = try createTree(pool, allocator, depth - 1);
    } else {
        node.left = null;
        node.right = null;
    }
...
pub fn main(init: std.process.Init) !void {
    ...
    const total_nodes = (@as(usize, 1) << @as(u6, @intCast(depth + 1))) - 1;

    var pool = try std.heap.MemoryPool(Node).initCapacity(std.heap.page_allocator, total_nodes);
    defer pool.deinit(std.heap.page_allocator);

    const root = try createTree(&pool, std.heap.page_allocator, depth);
