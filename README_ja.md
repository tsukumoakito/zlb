...
| **sieve** | エラトステネスの篩 | 配列アクセス速度、BitSetのデータ密度、境界チェックの影響 |
| **btree** | 二分木の動的生成・破棄 | 各種アロケータ（Arena, Pool, Fixed）の効率とGCのコスト |
| **log_proc** | 構造化ログ処理 | 文字列探索、JSONパース、ゼロ・アロケーション書式化の性能 |
| **atomics** | 原子操作と同期プリミティブ | ハードウェアレベルの原子操作とユーザー空間同期のオーバーヘッド |
